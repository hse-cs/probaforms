...
class BaseBackbone(nn.Module):
    ...
    def __init__(self, var_dim, cond_dim=None, hid_dim=64, n_steps=200,
                 steps_dim=32, steps_out_dim=None, steps_depth=4, act=nn.SiLU()):
        """
        Args:
            var_dim: size of target data
            cond_dim: size of conditional data
...
class PlainBackbone(BaseBackbone):
    ...
    """
    def __init__(self, var_dim, cond_dim=None, hid_dim=64, num_blocks=6, n_steps=200,
                 steps_dim=32, steps_out_dim=None, steps_depth=4, act=nn.SiLU()):
        """
        Args:
            var_dim: size of target data
            cond_dim: size of conditional data
            hid_dim: hidden size of model layers
        ...
        """
        super().__init__(var_dim, cond_dim, hid_dim, n_steps, steps_dim, steps_out_dim, steps_depth, act)
        self.num_blocks = num_blocks

        input_layer = nn.Linear(self.in_dim, hid_dim)  # in_dim = var_dim + cond_dir
...
class PlainBackboneResidual(PlainBackbone):
    ...
    def __init__(self, var_dim, cond_dim=None, hid_dim=64, num_blocks=6, n_steps=200,
        ...
        super().__init__(var_dim, cond_dim, hid_dim, num_blocks, n_steps, steps_dim, steps_out_dim, steps_depth, act)
